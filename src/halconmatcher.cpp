...
void HalconMatcher::init_model(std::string filename, float sampling_distance, std::string units){
    ...
    try{
       reference_object.ReadObjectModel3d(filename.c_str(), units.c_str(), "invert_normals", "true");
...
void HalconMatcher::match(){
  ...
  try{
    ROS_INFO("Thresholding by distance");
    thresholded = scene.SelectPointsObjectModel3d("&distance", 0.004, 0.10);
  ...
  try{
    ...
    hv_ObjectModel3DSelected = scene.SelectObjectModel3d(connected, "num_points", "and", 1000, "max");
  }catch(HOperatorException &e){
    ROS_ERROR("Selecting objects: %s", e.ErrorMessage().Text());
    return;
  }

  ROS_INFO("Finding surface model");
